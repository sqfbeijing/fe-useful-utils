;(function(global) {
   ...
   * @param {number}
   */
  Util.FORM.isLegalLandline = (number, reg = /^([0-9]{3,4}-)?[0-9]{7,8}$/) => {
    if (reg.test(number)) {
      return true
    }
    return false
  ...
  // 		return {};
  // 	}
  // 	str = url.split('?')[1]; // a=2&b=3#213
  // 	if (str.indexOf('#') !== -1) {
  // 		str = str.split('#')[0]; // a=2&b=3
  // 	}
  // 	if (str.indexOf('&') !== -1) {
  // 		arr = str.split('&');
  // 	} else {
  ...
  // console.log("17:18", parseQueryString(location.href));
