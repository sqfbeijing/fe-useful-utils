...
export const getHMSBySecond = (allSecond) => {
	...
	const minute = Math.floor((allSecond - hour * 3600) / 60);
	const second = Math.floor(allSecond - hour * 3600 - minute * 60);
	// 以冒号分隔  00:32:09
	const string = `${formatNumber(hour)}:${formatNumber(minute)}:${formatNumber(second)}`;
...
export const getMSBySecond = (allSecond) => {
	...
	const minute = Math.floor(allSecond / 60);
	const second = allSecond - minute * 60;
	// 以冒号分隔  00:32
	const string = `${formatNumber(minute)}:${formatNumber(second)}`;
